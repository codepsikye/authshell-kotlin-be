...
class MigrationValidationTest {
    ...
    @Test
    fun `should verify migration adds correct columns with proper constraints`() {
        dataSource.connection.use { connection ->
            targetTables.forEach { tableName ->
                // Verify created_at column exists and has proper constraints
                val createdAtQuery = """
                    SELECT column_name, is_nullable, column_default, data_type
                    FROM information_schema.columns 
                    WHERE table_name = UPPER(?) AND column_name = UPPER('created_at')
                """.trimIndent()
                
                connection.prepareStatement(createdAtQuery).use { stmt ->
                    stmt.setString(1, tableName)
                    val rs = stmt.executeQuery()
                    assertTrue(rs.next(), "created_at column should exist in $tableName table")
                    assertEquals("CREATED_AT", rs.getString("column_name"))
                    assertEquals("NO", rs.getString("is_nullable"), "created_at should be NOT NULL in $tableName")
                    // Note: H2 may not show default values for timestamp columns in information_schema
                    assertTrue(rs.getString("data_type").contains("TIMESTAMP", ignoreCase = true), 
                        "created_at should be TIMESTAMP type in $tableName")
                }

                // Verify updated_at column exists and has proper constraints
                val updatedAtQuery = """
                    SELECT column_name, is_nullable, column_default, data_type
                    FROM information_schema.columns 
                    WHERE table_name = UPPER(?) AND column_name = UPPER('updated_at')
                """.trimIndent()
                
                connection.prepareStatement(updatedAtQuery).use { stmt ->
                    stmt.setString(1, tableName)
                    val rs = stmt.executeQuery()
                    assertTrue(rs.next(), "updated_at column should exist in $tableName table")
                    assertEquals("UPDATED_AT", rs.getString("column_name"))
                    assertEquals("NO", rs.getString("is_nullable"), "updated_at should be NOT NULL in $tableName")
                    assertTrue(rs.getString("data_type").contains("TIMESTAMP", ignoreCase = true), 
                        "updated_at should be TIMESTAMP type in $tableName")
                }
            }
        }
    }
