...
class AccessRightTest {
    ...
    fun `should create AccessRight with required parameters`() {
        ...
        assertNotNull(accessRight.createdAt)
        assertNotNull(accessRight.updatedAt)
    }
    ...
    fun `should use default values for timestamps if not provided`() {
        ...
        accessRight.createdAt?.let { createdAt ->
            assert(createdAt.truncatedTo(ChronoUnit.SECONDS) >= beforeCreation)
            assert(createdAt.truncatedTo(ChronoUnit.SECONDS) <= afterCreation)
        }
        accessRight.updatedAt?.let { updatedAt ->
            assert(updatedAt.truncatedTo(ChronoUnit.SECONDS) >= beforeCreation)
            assert(updatedAt.truncatedTo(ChronoUnit.SECONDS) <= afterCreation)
        }
    }
